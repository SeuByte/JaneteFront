...
const produtosDestaque = [
  { id: "23432252", nome: "Mel de Abelha Orgânico", preco: "R$25.99", img: meldelicia },
  { id: "23432253", nome: "Óleo de Coco Extra Virgem", preco: "R$25.99", img: cocodelicia },
  { id: "23432254", nome: "Sabonete de Lavanda", preco: "R$25.99", img: abelhalavanda },
];
...
export default function Hero() {
  ...
  return (
    <section className="w-full">
      ...
      <div id="produtos-destaque" className="bg-white pb-6 xs:pb-8 sm:pb-10 lg:pb-12 w-full">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-5 xs:mb-6 sm:mb-8">
            <h2 className="text-lg xs:text-xl sm:text-2xl lg:text-3xl font-bold text-gray-800">Produtos em Destaque</h2>
            <p className="text-gray-500 text-[11px] xs:text-xs sm:text-sm mt-1">Os queridinhos dos nossos clientes</p>
          </div>
          ...
          <CarrosselDots
            ...
            dotColor="#147C06"
            renderItem={(produto) => (
              <div key={produto.id} className="bg-gray-50 p-3 flex flex-col gap-2 rounded-2xl border border-gray-100">
                <div className="h-28 xs:h-36 w-full overflow-hidden rounded-xl bg-white flex items-center justify-center">
                  <img src={produto.img} alt={produto.nome} className="w-full h-full object-contain p-2" />
                </div>
                <div className="flex flex-col gap-2 xs:gap-3">
                  <div className="flex flex-row justify-between items-start gap-1">
                    <div className="flex flex-col min-w-0">
                      <span className="text-[11px] xs:text-xs font-bold leading-tight truncate">{produto.nome}</span>
                      <p className="text-[9px] xs:text-[10px] text-gray-500">ID: {produto.id}</p>
                    </div>
                    <span className="font-bold text-green-600 text-[11px] xs:text-xs shrink-0">{produto.preco}</span>
                  </div>
                  <button className="hover:bg-green-700 text-gray-50 bg-green-500 py-1.5 xs:py-2 rounded-md cursor-pointer text-[10px] xs:text-xs transition-colors">
          ...
          <div className="hidden lg:grid grid-cols-3 gap-5">
            {produtosDestaque.map((produto) => (
              <div key={produto.id} className="bg-gray-50 p-3 flex flex-col gap-1 rounded-2xl border border-gray-100 hover:shadow-md transition-shadow">
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex flex-row justify-between">
                    <div className="flex flex-col">
                      ...
                      <p className="text-xs text-gray-700">ID: {produto.id}</p>
                    </div>
                    <span className="font-bold text-green-600">{produto.preco}</span>
                  </div>
          ...
          <div className="text-center mt-6 xs:mt-8">
            <a href="/produtos" className="inline-block border border-[#147C06] text-[#147C06] hover:bg-[#147C06] hover:text-white font-semibold px-5 xs:px-6 py-2 xs:py-2.5 rounded-full transition-colors text-xs xs:text-sm">
